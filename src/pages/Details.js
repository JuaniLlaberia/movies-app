...
const Details = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`${apiConfig.baseUrl}${type}/${id}?language=en-US&api_key=${apiConfig.apiKey}`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`${apiConfig.baseUrl}${type}/${id}/recommendations?language=en-US&page=1&api_key=${apiConfig.apiKey}`);
                const data = await response.json();
    ...
    const recommendationsToRender = recommendations?.map(rec => {
        let title = rec?.title
  ...
  <>
    <section className='details-page'>
      ...
      <div className='details-page-info-section'>
        ...
        <div className='img-container'><img src={apiConfig.posterImgUrl(info.poster_path)} className='details-page-poster'/></div>
      </div>
    </section>
    <section className='recommendations'>
        <h6 className='rec-title'>Recommendations</h6>
        <ul className='trending-items-scroll'>
            {recommendationsToRender}
        </ul>
    </section>
  </>
  )
